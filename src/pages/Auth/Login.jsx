...
const Login = () => {
  ...
  const loginHandle = async (e) => {
    ...
    setLoading(true);
    if (email !== '' && password !== '') {
      await axios.post('https://pmb-api.politekniklp3i-tasikmalaya.ac.id/auth/login/v1', {
        email: email,
        password: password,
      }, {
      ...
      loading ? (
        ...
        <main className='flex flex-col items-center justify-center bg-gradient-to-b from-lp3i-400 via-lp3i-200 to-lp3i-400 h-screen p-5 space-y-5'>
          ...
          <section className='max-w-lg mx-auto text-center space-y-1'>
            <h2 className='font-bold text-3xl text-white drop-shadow'>SBPMB Online: Tes Beasiswa</h2>
            <p className='text-white text-sm drop-shadow'>Raih kesempatan emas untuk mendapatkan beasiswa! Platform tes beasiswa online LP3I dirancang khusus untuk membantu siswa/i terbaik mewujudkan masa depan gemilang.</p>
          </section>
