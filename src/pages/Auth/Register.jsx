...
const Register = () => {
  ...
  const registerHandle = async () => {
    ...
    if (whatsapp !== '' && email !== '' && name !== '' && year !== '') {
      ...
      if (confirmed) {
        await axios.post('https://api.politekniklp3i-tasikmalaya.ac.id/pmb/auth/register/v3', data, {
                  ...
                  </ul>
                  {
                    whatsappMessage ? (
                      ...
                      <p className="mt-2 text-xs text-red-500">
                        ...
                        <a href="https://wa.me?phone=6282219509698&text=Kirim%20pesan%20perintah%20ini%20untuk%20reset%20password%20:resetpass:" target='_blank' className='underline' rel="noreferrer">lupa kata sandi?</a>
                      </p>
                    ) : null
                  }
                </div>
                ...
                <form onSubmit={(e) => checkValidation(e, 'email')} method='POST' className='space-y-4'>
                  <div className='flex items-center gap-4'>
                    ...
                    <div className='w-full'>
                      ...
                      <div className='flex gap-2'>
                        <input type="number" value={year} onChange={(e) => setYear(e.target.value)} className="bg-gray-50 border-2 border-lp3i-100 outline-none text-gray-900 text-sm rounded-xl focus:none block w-full px-4 py-2.5" placeholder="Tahun lulus" required={true} />
                      </div>
                      <ul className="ml-2 mt-2 text-xs text-red-600 list-disc">
                        {
